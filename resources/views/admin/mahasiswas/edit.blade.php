...
<x-app-layout>
    <div class="mx-auto max-w-lg">
        <form method="POST" action="{{ route('admin.mahasiswa.update', $mahasiswa) }}" class="space-y-5 rounded-lg border border-slate-200 bg-white p-6">
            @csrf
            @method('PUT')

            <div>
                <h3 class="font-display text-lg font-semibold text-navy">Data Mahasiswa</h3>
                <p class="mt-1 text-sm text-slate">Perbarui akun mahasiswa dan data akademiknya.</p>
            </div>
